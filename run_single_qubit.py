...
from single_qubit import get_random_observable, circuit, circuit_with_opg, finite_diff_first, evaluate_on_grid
...
def exact_grad():
    ...
    grads_a_generator = evaluate_on_grid(grad_fn_generator, a_grid, b_lines, jnp.array(0.), observable=observable)
    grads_a_exact = evaluate_on_grid(grad_fn_autodiff, a_grid, b_lines, observable=observable)
    grads_a_finite_diff = evaluate_on_grid(grad_fn_finite_diff, a_grid, b_lines, observable=observable)
    ...
    for i in range(3):
        ...
        axs.plot(a_grid, grads_a_finite_diff[:, i], lw=2, label=rf"$b = ${b_lines[i]:1.1f}", marker='o', ls='', markersize=4)
...
def sampled_grad(shots):
    ...
    qnode_grad_sample_ps = qml.QNode(circuit_with_opg, dev_sample, interface="jax", diff_method="parameter-shift")
    ...
    grads_a_exact = evaluate_on_grid(grad_fn_generator, a_grid, b_lines, jnp.array(0.), observable=observable)
    grads_a_sample_ps_mean, grads_a_sample_ps_std = evaluate_on_grid(grad_fn_sample_ps, a_grid, b_lines, jnp.array(0.), observable=observable, sampled=True)
    ...
    for i in range(3):
        ...
        axs.errorbar(a_grid, grads_a_sample_ps_mean[:, i], yerr=grads_a_sample_ps_std[:, i],
                     label=rf"$b = ${b_lines[i]:1.1f}", lw=2, alpha=0.8,
                     zorder=0)
...
def parshift_vs_finitediff(shots):
    ...
    grads_a_exact = evaluate_on_grid(grad_fn_generator, a_grid, b_lines, jnp.array(0.), observable=observable)
    grads_a_sample_ps_mean, grads_a_sample_ps_std = evaluate_on_grid(grad_fn_sample_ps, a_grid, b_lines, jnp.array(0.), observable=observable, sampled=True)
    grads_a_sample_fd_mean, grads_a_sample_fd_std = evaluate_on_grid(grad_fn_sample_fd, a_grid, b_lines, observable=observable, sampled=True)

    plt.style.use('science')
    plt.rcParams.update({'font.size': 13})
    fig, axs = plt.subplots(3, 1, figsize=(5, 8), gridspec_kw={"hspace":0.})
    labels = ["Parameter shift", "Finite difference"]
    colors = ["xkcd:bright blue", "xkcd:burnt orange"]
    for i, ax in enumerate(axs):
        plot = ax.plot(a_grid, grads_a_exact[:, i], lw=1.1, ls="-", zorder=3, alpha=0.7, c="k", label="" if i<2 else "Exact")[0]
        for j, (mean, std) in enumerate([(grads_a_sample_ps_mean, grads_a_sample_ps_std), (grads_a_sample_fd_mean, grads_a_sample_fd_std)]):
            c = colors[j]
            mu, sigma = mean[:, i], std[:, i]
            ax.plot(a_grid, mu, lw=2.5, ls="--", zorder=1, label="" if i<2 else labels[j], c=c)
            ax.plot(a_grid, mu+sigma, lw=1., ls="-", alpha=0.3, c=c, zorder=-1)
            ax.plot(a_grid, mu-sigma, lw=1., ls="-", alpha=0.3, c=c, zorder=-1)
            ax.fill_between(a_grid, mu-sigma, mu+sigma, color=c, alpha=0.2, zorder=-1)

        ax.text(0.05, 0.95, rf"$b = ${b_lines[i]:1.1f}", transform=ax.transAxes, verticalalignment="top")
